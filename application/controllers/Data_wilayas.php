...
class Data_wilayas extends CI_Controller
{    
    ...
    {
            $this->load->view('admin/index_willaya'); 
  ...
  // DataTable data
    ...
    $sel = $this->db->query("select count(*) as allcount from wilayas");
    $records = $sel->row_array();
    $totalRecords = $records['allcount'];

    ## Total number of records with filtering
    $sel = $this->db->query("select count(*) as allcount from wilayas WHERE 1 ".$searchQuery);
    $records = $sel->row_array();
    $totalRecordwithFilter = $records['allcount'];

    ## Fetch records
    $empQuery = "select * from wilayas WHERE 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$start.",".$rowperpage;
...
// Fetch  details
    ...
    $record = $this->db->query("SELECT * FROM wilayas WHERE id=".$id);
    ...
    public function Update()
{
    ...
    // Check id
    $record = $this->db->query("SELECT id FROM wilayas WHERE id=".$id);
    if($record->num_rows() > 0){
        ...
        if( $name_ar != '' && $name_en != '' && $latitude != '' && $longitude != '' ){
            ...
            $this->db->query("UPDATE wilayas SET code='".$wilaya_id."',name_ar='".$name_ar."',name_en='".$name_en."',latitude='".$latitude."',longitude='".$longitude."' WHERE id=".$id);
...
// Delete 
    ...
    if($record->num_rows() > 0){
       ...
       // $this->db->query("DELETE FROM wilayas WHERE id=".$id."");
           $this->db->where('id', $id);
