...
<div class="container-fluid">
 <h1 class="display-4 mt-5 mb-5 border-bottom border-dark pr-5">تعديل دائرة </h1>
 ...
 <div class="col">
       ...
       var userDataTable =  $('#userTable').DataTable({
            ...
            serverSide: true,
            serverMethod: 'post',
            ajax: '<?php echo base_url(); ?>Data_test/getTable',
            columns: [{
                    "data": "id"
                },
                {
            ...
            columnDefs: [{
                ...
                },
                {
                    ...
                    "render": function(data, type, row, meta) {
                        return '<button class="btn btn-sm btn-info updateUser" data-toggle="modal" data-target="#updateModal" data-id="' + row.id + '" data-title="' + row.name_en + '" data-slug="' + row.name_ar + '" data-text="' + row.notice + '">تعديل</button><button class="btn btn-sm mr-3 btn-danger deleteUser" data-id="' + row.id + '">حذف</button>';
                    }
                }
            ],
     ...
     /*    var userDataTable = $('#userTable').DataTable({
                ...
                'serverMethod': 'post',
                'ajax': {
                    'url':'<?php echo base_url(); ?>Data/getTable',
					 "data": function ( d ) {
...
*/
            ...
            $('#userTable').on('click','.updateUser',function(){
                ...
                // AJAX request
                $.ajax({
                    url: '<?php echo base_url(); ?>Data_test/Fetch',
                    type: 'post',
                    data: {id: id},
                    dataType: 'json',
            ...
            $('#btn_save').click(function(){
                ...
                if(name_ar !='' && name_en != '' && wilayas != ''){
                    ...
                    // AJAX request
                    $.ajax({
                        url: '<?php echo base_url(); ?>Data_test/Update',
                        type: 'post',
 ...
 // Delete record
          ...
          $('#userTable').on('click','.deleteUser',function(){
                ...
                if (deleteConfirm == true) {
                    $.ajax({
                        url:'<?php echo base_url();?>Data_test/deletes',
                        type:'post',
                        data:{ id : id },
						dataType: 'html',
                        success: function(response){
...
<script>
        ...
        // load wilayas
        ajax('<?php echo base_url();?>Data_test/Fetch_wilayas', (res) => {
